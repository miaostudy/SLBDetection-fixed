...
def to_2tuple(x):  # 自己实现的to_2tuple，因为timm没有安装
    """Convert a value to a 2-tuple. If x is already a tuple/list, return it; otherwise return (x, x)."""
...
class WaveletDownsample(nn.Module):
    """
    Haar 小波下采样，保留 LL/LH/HL/HH 四个子带后再做 1x1 卷积整合。
    用于替换早期 stride=2 下采样，减少小目标高频信息丢失。
    """

    def __init__(self, c1, c2=None, keep_ll=True, act=True):
        super().__init__()
        self.keep_ll = keep_ll
        # 固定 Haar 核
        haar = torch.tensor([[1., 1.],
        ...
        hh = torch.tensor([[1., -1.],
                           [-1., 1.]], dtype=torch.float32) / 2.0
        self.register_buffer('haar_kernels', torch.stack([haar, lh, hl, hh], dim=0).unsqueeze(1))
        out_channels = c2 if c2 is not None else c1 * (1 if keep_ll else 4)
        self.post = Conv(4 * c1, out_channels, k=1, s=1, act=act)

    def forward(self, x):
        b, c, h, w = x.shape
        # 组卷积：每个输入通道共享同一组 4 个 Haar 核
        weight = self.haar_kernels.repeat(c, 1, 1, 1)  # (4*c,1,2,2)
        bands = F.conv2d(x, weight, stride=2, groups=c)  # (b,4c,h/2,w/2)
        bands = bands.view(b, c, 4, bands.shape[-2], bands.shape[-1]).permute(0, 2, 1, 3, 4)
        bands = bands.reshape(b, 4 * c, bands.shape[-2], bands.shape[-1])
        if self.keep_ll:
            # 仅保留 LL + 细节（直接输出全部 4 子带，后续 1x1 卷积融合）
            return self.post(bands)
        else:
            # 如果只需要高频，可以剔除 LL
            bands = bands[:, c:, ...]
            return self.post(bands)
...
class FrequencyChannelAttention(nn.Module):
    """
    简化版 FcaNet：在通道维上利用频域统计自适应重标定。
    """

    def __init__(self, channels, reduction=8, topk=0):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.topk and self.topk > 0:
            ...
            stat = topk.mean(dim=-1)
        else:
            stat = mag.mean(dim=(2, 3))
        w = torch.sigmoid(self.mlp(stat)).view(b, c, 1, 1)
        return x * w
...
class Concat(nn.Module):
    ...
    def forward(self, x):
        ...
        # 如果是沿着通道维度连接（dimension=1），且空间尺寸不同，则调整到相同尺寸
        if self.d == 1:  # 通道维度
            # 获取所有张量的空间尺寸
            spatial_sizes = [(s[2], s[3]) for s in shapes]  # (H, W)

            # 检查是否所有张量的空间尺寸都相同
            if not all(size == spatial_sizes[0] for size in spatial_sizes):
                # 找到最大尺寸
                max_h = max(s[0] for s in spatial_sizes)
                max_w = max(s[1] for s in spatial_sizes)

                # 调整所有张量到相同的空间尺寸
                adjusted_x = []
                for i, tensor in enumerate(x):
                    curr_h, curr_w = spatial_sizes[i]
                    if curr_h != max_h or curr_w != max_w:
                        # 使用插值调整到最大尺寸
                        resized_tensor = F.interpolate(tensor, size=(max_h, max_w), mode='nearest')
                        adjusted_x.append(resized_tensor)
                    else:
                        adjusted_x.append(tensor)

                return torch.cat(adjusted_x, self.d)
            else:
                # 所有张量尺寸相同，直接连接
            ...
            try:
                ...
                print("===== Concat 出错 =====")
                print("层索引 i:", getattr(self, "i", None))
                print("from f:", getattr(self, "f", None))
                print("输入形状:", shapes)
                raise e
...
class RepConv_OREPA(nn.Module):
    ...
    def get_custom_L2(self):
        ...
        l2_loss_circle = (K3 ** 2).sum() - (K3[
                                                :, :, 1:2, 1:2] ** 2).sum()  # The L2 loss of the "circle" of weights in 3x3 kernel. Use regular L2 on them.
...
class LBASwinTransformerblock(nn.Module):
    def __init__(self, c1, c2, num_heads, num_layers, window_size=8, freq_attention=False):
        super().__init__()
        self.conv = None
        if c1 != c2:
            self.conv = Conv(c1, c2)
        self.freq_attention = freq_attention
        self.freq_ca = FrequencyChannelAttention(c2) if freq_attention else None
